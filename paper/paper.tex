...
Figure~\ref{fig:lifecycle_previous} illustrates the interval-based current state along the customer journey from vehicle operation through service execution to post-repair follow-up, identifying the actors involved at each stage and the points at which reactive servicing causes value leakage. The processes are separated into a physical and a digital lane, making the extent of digital involvement at each stage directly comparable. The corresponding DT/PdM-enabled target state is presented separately in Figure~\ref{fig:lifecycle_future}.
