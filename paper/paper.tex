...
normal driving. The lifecycle is initiated by the customer rather than the
dealership: it begins with the customer's contact to arrange the service
appointment, and the subsequent steps remain largely in the customer's hands.
The trigger is either a time- or mileage-triggered reminder or an acute fault
such as a warning lamp. Both are late-stage: the mileage interval is set
conservatively, so most vehicles still have substantial remaining part life,
while the fault-driven trigger often arrives after the wear limit has already
been exceeded~\cite{abougrad2026xai}. Most of these steps are controlled by
the customer, while only a few are captured digitally, since no data basis on
the vehicle's component state exists. Because no
proactive outreach precedes the fault, the customer exercises free choice of
workshop at the moment of highest urgency, and the majority of these unplanned
repairs are captured by independent third-party garages~\cite{dat2026report}.
...
years~\cite{dat2026report}---and with cost-driven customer behaviour: 43\,\% of
German vehicle owners now perform only the minimum maintenance~\cite{dat2026report}.
The dealership's contact with the customer is therefore reduced to the
statutory inspection cycle, limiting both retention and upselling opportunity.
Moreover, most of these steps are physical in nature and, in the absence of a
data source capturing them, cannot be represented in the digital lane.
...
The DT is the core enabler of this digital process layer. It allows
the customer to be involved only to the extent necessary, raising the customer
experience compared to the current state. The lifecycle is now initiated and
steered by the PdM forecast rather than by the customer. Not only the
required documents but also the processes themselves become digitally
traceable, and the resulting data can additionally be analysed for upselling,
increasing service revenue. Nevertheless, some processes, such as the physical
inspection, cannot be digitalised by DT and PdM.
In the DT/PdM-enabled target state, the vehicle's digital twin continuously
aggregates sensor streams, OBD signals, operating parameters, and
environmental conditions via OTA telemetry to maintain an
